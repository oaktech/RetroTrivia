...
extension TriviaQuestion {
    ...
    /// - Parameter sortOrder: Optional sort order for random sampling (0-9999)
    ...
    func toCloudKitRecord(sortOrder: Int? = nil) -> CKRecord {
        ...
        // sortOrder enables efficient random sampling for large datasets (10K+)
        record["sortOrder"] = Int64(sortOrder ?? Int.random(in: 0...9999))
        return record
    }
}
